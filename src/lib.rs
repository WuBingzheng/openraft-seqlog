...
use openraft::{LogState, OptionalSend, RaftLogReader, RaftTypeConfig};

use seqlog::{SeqLog, SeqLogReader, SeqLogSyncer};

pub struct SeqLogStore<C, Codec>
where
    C: RaftTypeConfig,
    Codec: EntryCodec<C::Entry>,
{
    store: SeqLog,
    codec: Codec,
    write_buf: Vec<u8>,
...
pub trait EntryCodec<E>: Send + OptionalSync + 'static {
    type Error;

    fn encode(&self, entry: &E, buf: &mut Vec<u8>) -> Result<(), Self::Error>;

    fn decode(&self, data: &[u8]) -> Result<E, Self::Error>;
}
...
impl<C, Codec> RaftLogStorage<C> for SeqLogStore<C, Codec>
where
    C: RaftTypeConfig,
    Codec: EntryCodec<C::Entry>,
{
        ...
        I::IntoIter: OptionalSend,
    {
        ...
        for entry in entries.into_iter() {
            let _ = self.codec.encode(&entry, &mut self.write_buf);
